...
X_big_int_exp_intcep<-cbind(1,log(exp_index),X_big_int)
...
data_phenotype_order<-data_phenotype[,order(total)]
head(data_phenotype_order)
i_35_0<-which(data_phenotype_order[1,]==35 & data_phenotype_order[2,]==0)
i_35_1<-which(data_phenotype_order[1,]==35 & data_phenotype_order[2,]==1)
i_35_7<-which(data_phenotype_order[1,]==35 & data_phenotype_order[2,]==7)
i_35_28<-which(data_phenotype_order[1,]==35 & data_phenotype_order[2,]==28)

####################################################
####construct the data we need to plot for subject 35: 0->1
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_35_0+2]<-gene_whole[,i_35_1+2]-gene_whole[,i_35_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_35_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_35_0)
{
  #geneate link data with 25th tissue
  ...
  nodes_gene<-data.frame(self_size[,c(1,(i+1))])
  colnames(nodes_gene)<-c("id","self_weight")
  nodes_gene$node_type<-ifelse(nodes_gene$id %in% c(1,53,59,109),1,ifelse(nodes_gene$id %in% (setdiff(self_size[,1],self_index)),2,3))
  net <- graph.data.frame(links_gene, nodes_gene, directed=T) 
  class(net)
  net 
  ...
  V(net)$size <- (V(net)$self_weight)^0.4*3.5
  #initial_weight<-abs(gene_whole[,c(i_40_1+2)]-E(net)$tissue_name)
  E(net)$width <- abs(gene_whole_threhold[,i+2])*200
  ...
  #l <- layout.norm(l, ymin=-1, ymax=1, xmin=-1, xmax=1)
  #V(net) #total 34 clusters included in the network
  plot(net.bg, rescale=F, layout=l,edge.arrow.size=.07,mark.col=c("#E5E5E7","#ECD89C","#C5E5E7","#CCD89A")) #mark.border=NA)
}


####################################################
####construct the data we need to plot for subject 35: 0->7
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_35_0+2]<-gene_whole[,i_35_7+2]-gene_whole[,i_35_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_35_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_35_0)
{
...
####################################################
####construct the data we need to plot subject 35: 0->28
####################################################
#lines(x=exp_index_scale,y=fitted,ylab="y")
dim(gene_whole)
unique(gene_whole[,1])
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_35_0+2]<-gene_whole[,i_35_28+2]-gene_whole[,i_35_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_35_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_35_0)
{
...
####################################################
#subject:success subject34
####################################################
###order phenotype
total<-apply(data_gene_exp,2,sum)
data_phenotype_order<-data_phenotype[,order(total)]
head(data_phenotype_order)
i_34_0<-which(data_phenotype_order[1,]==34 & data_phenotype_order[2,]==0)
i_34_1<-which(data_phenotype_order[1,]==34 & data_phenotype_order[2,]==1)
i_34_7<-which(data_phenotype_order[1,]==34 & data_phenotype_order[2,]==7)
i_34_28<-which(data_phenotype_order[1,]==34 & data_phenotype_order[2,]==28)

####################################################
####construct the data we need to plot for subject 34: 0->1
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_34_0+2]<-gene_whole[,i_34_1+2]-gene_whole[,i_34_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_34_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_34_0)
{
  #geneate link data with 25th tissue
...
####################################################
####construct the data we need to plot for subject 34: 0->7
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_34_0+2]<-gene_whole[,i_34_7+2]-gene_whole[,i_34_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_34_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_34_0)
{
...
###try not circle subject 34

####################################################
####construct the data we need to plot subject 34 0->28
####################################################
#lines(x=exp_index_scale,y=fitted,ylab="y")
dim(gene_whole)
unique(gene_whole[,1])
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_34_0+2]<-gene_whole[,i_34_28+2]-gene_whole[,i_34_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_34_0+2])>0.001,]
dim(gene_whole_threhold)
for (i in i_34_0)
{
...
####################################################
#subject:success subject 32 draw graph!!!!!!
####################################################
###order phenotype
total<-apply(data_gene_exp,2,sum)
data_phenotype_order<-data_phenotype[,order(total)]
head(data_phenotype_order)
i_32_0<-which(data_phenotype_order[1,]==32 & data_phenotype_order[2,]==0)
i_32_1<-which(data_phenotype_order[1,]==32 & data_phenotype_order[2,]==1)
i_32_7<-which(data_phenotype_order[1,]==32 & data_phenotype_order[2,]==7)
i_32_28<-which(data_phenotype_order[1,]==32 & data_phenotype_order[2,]==28)

####################################################
####construct the data we need to plot for subject 32: 0->1
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_32_0+2]<-gene_whole[,i_32_1+2]-gene_whole[,i_32_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_32_0+2])>0.005,]
dim(gene_whole_threhold)
for (i in i_32_0)
{
  #geneate link data with 25th tissue
  ...
  nodes_gene<-data.frame(self_size[,c(1,(i+1))])
  colnames(nodes_gene)<-c("id","self_weight")
  nodes_gene$node_type<-ifelse(nodes_gene$id %in% c(86,53,5,109),1,ifelse(nodes_gene$id %in% (setdiff(self_size[,1],self_index)),2,3))
  net <- graph.data.frame(links_gene, nodes_gene, directed=T) 
  class(net)
  net 
  ...
  l[40,2]<-0.45
  l[46,2]<-0.65
  l[54,2]<-l[84,2]<-l[61,2]<-l[131,2]<-0.7
  l[121,2]<-0.65
  l[89,2]<-0.4
  l[99,1]<-0.6
  l[68,1]<-l[57,1]<-0.6
...
####################################################
####construct the data we need to plot for subject 32: 0->7
####################################################
...
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_32_0+2]<-gene_whole[,i_32_7+2]-gene_whole[,i_32_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_32_0+2])>0.005,]
dim(gene_whole_threhold)
for (i in i_32_0)
{
  ...
  nodes_gene<-data.frame(self_size[,c(1,(i+1))])
  colnames(nodes_gene)<-c("id","self_weight")
  nodes_gene$node_type<-ifelse(nodes_gene$id %in% c(53,109),1,ifelse(nodes_gene$id %in% (setdiff(self_size[,1],self_index)),2,3))
  net <- graph.data.frame(links_gene, nodes_gene, directed=T) 
  class(net)
  net 
...
###try not circle subject 32

####################################################
####construct the data we need to plot subject 32 0->28
####################################################
#lines(x=exp_index_scale,y=fitted,ylab="y")
dim(gene_whole)
unique(gene_whole[,1])
table(gene_whole[,1])
gene_whole_threhold<-gene_whole
gene_whole_threhold[,i_32_0+2]<-gene_whole[,i_32_28+2]-gene_whole[,i_32_0+2]
gene_whole_threhold<-gene_whole_threhold[abs(gene_whole_threhold[,i_32_0+2])>0.005,]
dim(gene_whole_threhold)
for (i in i_32_0)
{
...
data_phenotype_order<-data_phenotype[,order(total)]
head(data_phenotype_order)
s0<-(data_phenotype_order[2,]==0 &data_phenotype_order[3,]==1)
s1<-(data_phenotype_order[2,]==1 &data_phenotype_order[3,]==1)
s7<-(data_phenotype_order[2,]==7 &data_phenotype_order[3,]==1)
s28<-(data_phenotype_order[2,]==28 &data_phenotype_order[3,]==1)
f0<-(data_phenotype_order[2,]==0 &data_phenotype_order[3,]==0)
f1<-(data_phenotype_order[2,]==1 &data_phenotype_order[3,]==0)
f7<-(data_phenotype_order[2,]==7 &data_phenotype_order[3,]==0)
f28<-(data_phenotype_order[2,]==28 &data_phenotype_order[3,]==0)
...
self_size<-rep()
group<-c(0,rep(1:cluster,each=num_cov))
cluster_select<-c(20,27,118,135)
par(mfrow=c(2,2))
par(cex=1.5)
...
for (j in cluster_select)
{
  ...
  {
    plot(x=exp_index[s0],y=data_clustered[s0,gene_index],axes=FALSE,ylim=c(-1.4,max(data_clustered[,gene_index])+0.2),pch=19,col="pink1",xlim=c(min(exp_index),max(exp_index)+200))
    points(x=exp_index[s1],y=data_clustered[s1,gene_index],pch=19,cex=1.2,col="mediumorchid1")
    points(x=exp_index[s7],y=data_clustered[s7,gene_index],pch=19,cex=1.2,col="mediumpurple2")
    points(x=exp_index[s28],y=data_clustered[s28,gene_index],pch=19,cex=1.2,col="navyblue")
    points(x=exp_index[f0],y=data_clustered[f0,gene_index],pch=3,cex=1.2,col="pink1")
    points(x=exp_index[f1],y=data_clustered[f1,gene_index],pch=3,cex=1.2,col="mediumorchid1")
    points(x=exp_index[f7],y=data_clustered[f7,gene_index],pch=3,cex=1.2,col="mediumpurple2")
    points(x=exp_index[f28],y=data_clustered[f28,gene_index],pch=3,cex=1.2,col="navyblue")
    ...
    if (num_gene>0)
    {
      ...
      text(x=rep(max(exp_index)+3, length(influ_order)), y=seq(-0.5,2,length=length(influ_order)), pos=4, labels=influ_order)
    ...
    if (num_gene>0)
    {
      ...
      fitted_gene_matrix<-cbind(fitted_gene_matrix,fitted_gene)
      plot(x=exp_index,y=fitted_gene_matrix[,1],col="green",ylab='',xlab='',axes=FALSE,lwd=1.5,type="l",ylim=c(-0.1,0.6))
      ...
      axis(2,col = "grey40", col.axis = "grey20", at = c(0,0.6),cex.axis=0.7)
  ...
  {
    plot(x=exp_index[s0],y=data_clustered[s0,gene_index],axes=FALSE,ylim=c(-1,max(data_clustered[,gene_index])+0.2),pch=19,col="pink1",xlim=c(min(exp_index),max(exp_index)+200))
    points(x=exp_index[s1],y=data_clustered[s1,gene_index],pch=19,cex=1.2,col="mediumorchid1")
    points(x=exp_index[s7],y=data_clustered[s7,gene_index],pch=19,cex=1.2,col="mediumpurple2")
    points(x=exp_index[s28],y=data_clustered[s28,gene_index],pch=19,cex=1.2,col="navyblue")
    points(x=exp_index[f0],y=data_clustered[f0,gene_index],pch=3,cex=1.2,col="pink1")
    ...
    if (num_gene>0)
    {
      ...
      influ_order<-influ[order(fitted_gene_matrix[ncol(data_original),])]
      text(x=rep(max(exp_index)+3, length(influ_order)), y=seq(0,2,length=length(influ_order)), pos=4, labels=influ_order)
      box(col = "grey60",lwd=2)
      gene_cluster<-cbind(gene_cluster,j,t(fitted_gene_matrix))
      gene_whole<-rbind(gene_whole,gene_cluster)
    }
    ...
    if (num_gene>0)
    {
      ...
      fitted_gene<-X_big_int_exp_intcep[,index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]%*%beta_select[index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]
      fitted_gene_matrix<-cbind(fitted_gene_matrix,fitted_gene)
      plot(x=exp_index,y=fitted_gene_matrix[,1],ylab='',xlab='',axes=FALSE,col="green",lwd=1.5,ylim=c(-0.1,0.7),type="l")
      for (i in 2:num_gene)
      {
        gene_relate<-find_index[(i-1)*num_cov+1]%/%num_cov+1
        gene_cluster<-c(gene_cluster,gene_relate)
  ...
  {
    plot(x=exp_index[s0],y=data_clustered[s0,gene_index],axes=FALSE,ylim=c(-1,max(data_clustered[,gene_index])+0.6),pch=19,col="pink1",xlim=c(min(exp_index),max(exp_index)+200))
    points(x=exp_index[s1],y=data_clustered[s1,gene_index],pch=19,cex=1.2,col="mediumorchid1")
    points(x=exp_index[s7],y=data_clustered[s7,gene_index],pch=19,cex=1.2,col="mediumpurple2")
    points(x=exp_index[s28],y=data_clustered[s28,gene_index],pch=19,cex=1.2,col="navyblue")
    points(x=exp_index[f0],y=data_clustered[f0,gene_index],pch=3,cex=1.2,col="pink1")
    ...
    if (num_gene>0)
    {
      ...
      influ_order<-influ[order(fitted_gene_matrix[ncol(data_original),])]
      text(x=rep(max(exp_index)+3, length(influ_order)), y=seq(-1,3,length=length(influ_order)), pos=4, labels=influ_order)
    ...
    if (num_gene>0)
    {
      ...
      fitted_gene<-X_big_int_exp_intcep[,index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]%*%beta_select[index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]
      fitted_gene_matrix<-cbind(fitted_gene_matrix,fitted_gene)
      plot(x=exp_index,y=fitted_gene_matrix[,1],ylab='',xlab='',col="green",lwd=1.5,axes=FALSE,ylim=c(-0.3,0.1),type="l")
      for (i in 2:num_gene)
      {
        gene_relate<-find_index[(i-1)*num_cov+1]%/%num_cov+1
        gene_cluster<-c(gene_cluster,gene_relate)
      ...
      axis(2,col = "grey40", col.axis = "grey20", at = c(-0.3,0),cex.axis=0.7)
      #axis(1,col = "grey40", col.axis = "grey20", at = exp_index, labels=round(exp_index/10000,digits=1),cex.axis=0.7)
      box(col = "grey60",lwd=2)
    }
  } 
  ...
  if (j %in% (cluster_select[4]))
  {
    ...
    if (num_gene>0)
    {
      ...
      fitted_gene<-X_big_int_exp_intcep[,index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]%*%beta_select[index_nonzero[(3+(1-1)*num_cov):(3+1*num_cov-1)]]
      fitted_gene_matrix<-cbind(fitted_gene_matrix,fitted_gene)
      plot(x=exp_index,y=fitted_gene_matrix[,1],ylab='',xlab='', axes=FALSE, col="green",lwd=1.5,ylim=c(-0.55,0.1),type="l")
      ...
      axis(2,col = "grey40", col.axis = "grey20", at = c(-0.5,0),cex.axis=0.7)
      #axis(1,col = "grey40", col.axis = "grey20", at = exp_index, labels=round(exp_index/10000,digits=1),cex.axis=0.7)
      box(col = "grey60",lwd=2)
    }
  }
