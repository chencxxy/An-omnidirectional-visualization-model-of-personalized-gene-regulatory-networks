setwd("/Users/chixiangchen/Desktop/phd in psu/research/Prof. Wu/grn")
...
#read all data
data_grn<-read.csv("grn_more.csv",header = F,row.names = 1)
head(data_grn)
dim(data_grn)
str(data_grn)

#read gene expression 
data_gene_exp<-read.csv("grn.csv",header = F,row.names = 1)
head(data_gene_exp)
str(data_gene_exp)
...
#read outcome
data_outcome<-unlist(data_grn[3,])
data_outcome<-ifelse(data_outcome=="S",1,0)

#create patient and time
time<-rep(c(0,1,7,28),times=48)
patient<-rep(1:48,each=4)
data_phenotype<-rbind(patient,time,data_outcome)
head(data_phenotype)
head(data_gene_exp)
colnames(data_gene_exp)<-colnames(data_phenotype)<-rep(1:192)
...
#dim(data_derivative)
which(beta[2,]<0)
plot(x=exp_index,data_clustered[,140])
lines(x=exp_index,data_fitted[,140])
plot(x=exp_index,data_original[1840,])
...
write.csv(data_gene_cluster,"gene_cluster_information.csv")
...
####################################################################
#plot figures corresponding to clustered data of 10,30,110,129
####################################################################
...
representate<-c(200,420,1700,1840)
for (i in representate)
{
  ...
  if (i == representate[1])
  {
    plot(x=exp_index[s0],y=t(data_original)[s0,i],pch=19,cex=1.2,col="pink1",axes=FALSE,ylim=c(4.5,8.5),xlim=c(min(exp_index),max(exp_index)+300))
    points(x=exp_index[s1],y=t(data_original)[s1,i],pch=19,cex=1.2,col="mediumorchid1")
    points(x=exp_index[s7],y=t(data_original)[s7,i],pch=19,cex=1.2,col="mediumpurple2")
    points(x=exp_index[s28],y=t(data_original)[s28,i],pch=19,cex=1.2,col="navyblue")
    points(x=exp_index[f0],y=t(data_original)[f0,i],pch=3,cex=1.2,col="pink1")
    points(x=exp_index[f1],y=t(data_original)[f1,i],pch=3,cex=1.2,col="mediumorchid1")
    points(x=exp_index[f7],y=t(data_original)[f7,i],pch=3,cex=1.2,col="mediumpurple2")
    points(x=exp_index[f28],y=t(data_original)[f28,i],pch=3,cex=1.2,col="navyblue")
  ...
  if (i==representate[3])
  {
    plot(x=exp_index[s0],y=t(data_original)[s0,i],pch=19,cex=1.2,col="pink1",axes=FALSE,ylim=c(3,5.5),xlim=c(min(exp_index),max(exp_index)+300))
    points(x=exp_index[s1],y=t(data_original)[s1,i],pch=19,cex=1.2,col="mediumorchid1")
    points(x=exp_index[s7],y=t(data_original)[s7,i],pch=19,cex=1.2,col="mediumpurple2")
    points(x=exp_index[s28],y=t(data_original)[s28,i],pch=19,cex=1.2,col="navyblue")
    points(x=exp_index[f0],y=t(data_original)[f0,i],pch=3,cex=1.2,col="pink1")
    ...
    axis(2,col = "grey40", col.axis = "grey20", at = seq(3,6, 0.5),cex.axis=1.7)
    box(col = "grey60",lwd=2)
  }
  if(i==representate[4])
  {
  ...
  text(x=rep(max(exp_index)+3, 1), y=fitted[ncol(data_original)], pos=4, labels=row.names(data_original)[i],cex=0.88)
}
